...
const startDay = moment([yearForTargetMonth, targetMonth - 1, 1]);
const endDay = moment([yearForTargetMonth, targetMonth, 1]);
...
if (month === targetMonth && year === yearForTargetMonth) {
  const monthName = moment([year, month - 1, 1]).format("MMMM");
  const calendarDays = moment.weekdaysShort(); // Get short names of weekdays using Moment.js
  ...
  const daysInMonth = moment([year, month - 1]).daysInMonth(); // Get the number of days in the month
  const firstDay = moment([year, month - 1, 1]).day(); // Get the day of the week for the first day of the month
  let dateCounter = 1; // Start counting the dates from 1
}
