...
var currentMonth = currentDate.getUTCMonth() + 1; // Use getUTCMonth() for UTC month
...
for (var i = 1; i <= 6; i++) {
    ...
    var monthName = new Date(
        currentDate.getUTCFullYear(), // Use getUTCFullYear() for UTC year
        nextMonth - 1,
        1,
    ...
    var year =
        currentMonth + i > 12
        ? currentDate.getUTCFullYear() + 1 // Use getUTCFullYear() for UTC year
        : currentDate.getUTCFullYear(); // Use getUTCFullYear() for UTC year
...
var startDate = new Date(Date.UTC(firstMonthYear, monthIndex(firstMonthName), 1)); // Use Date.UTC() for UTC start date
var formattedStartDate = formatDate(startDate);

// Get the end date (last day of the first month plus one day)
var lastDay = new Date(Date.UTC(firstMonthYear, monthIndex(firstMonthName) + 1, 0)); // Use Date.UTC() for UTC end date
lastDay.setUTCDate(lastDay.getUTCDate() + 1); // Use setUTCDate() for UTC end date calculation
...
$("#months-container button:first").addClass("active");
  ...
  // Get the start date of the clicked month
    var startDate = new Date(Date.UTC(clickedYear, monthIndex(clickedMonth), 1)); // Use Date.UTC() for UTC start date
    var formattedStartDate = formatDate(startDate);

    // Get the end date (last day of the clicked month plus one day)
    var lastDay = new Date(Date.UTC(clickedYear, monthIndex(clickedMonth) + 1, 0)); // Use Date.UTC() for UTC end date
    lastDay.setUTCDate(lastDay.getUTCDate() + 1); // Use setUTCDate() for UTC end date calculation
    var formattedEndDate = formatDate(lastDay);
...
function monthIndex(month) {
    return new Date(Date.parse(month + " 1, 2023")).getUTCMonth(); // Use getUTCMonth() to obtain UTC month index
}
  ...
  const fetchData = async (lat, lon, dataType, temperatureUnit) => {
    ...
    const date = new Date(Date.UTC(
        item.date.getFullYear(),  // Use getFullYear() directly on the date object
        item.date.getMonth(),     // Use getMonth() directly on the date object
        item.date.getDate()       // Use getDate() directly on the date object
    ));
    const year = date.getUTCFullYear(); // Use getUTCFullYear() for UTC year
    const month = date.getUTCMonth() + 1; // Use getUTCMonth() for UTC month
    ...
    monthsData[key].push({
        day: date.getUTCDate(), // Use getUTCDate() for UTC day
        tmax: item.tmax,
        tmin: item.tmin,
        snow: item.snow,
        prcp: item.prcp,
    ...
    const monthName = new Date(Date.UTC(year, month - 1, 1)).toLocaleString("default", { month: "long" }); // Use Date.UTC() for UTC-based Date object
...
const daysInMonth = new Date(Date.UTC(year, month, 0)).getUTCDate(); // Use Date.UTC() for UTC-based Date object
const firstDay = new Date(Date.UTC(year, month - 1, 1)).getUTCDay(); // Use Date.UTC() for UTC-based Date object
let dateCounter = 1; // This doesn't require UTC modification
